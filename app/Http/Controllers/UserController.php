...
class UserController extends Controller
{
    ...
    public function datatable(Request $request)
    {
        ...
        $data = $users->map(function ($user) use ($roleColors) {
            ...
            return [
                ...
                'empresas' => $empresasHtml, // 👈 nueva columna
                'roles'    => $rolesHtml,
    ...
    public function suppliersDatatable(Request $request)
    {
        ...
        switch ($orderCol) {
            case 'company_name': $base->orderBy('s.company_name', $orderDir); break;
            case 'rfc':          $base->orderBy('s.rfc', $orderDir); break;
            case 'contact_person': $base->orderBy('s.contact_person', $orderDir); break;
            case 'contact_phone':  $base->orderBy('s.contact_phone', $orderDir); break;
            case 'email':        $base->orderBy('s.email', $orderDir); break;
            case 'bank_name':     $base->orderBy('s.bank_name', $orderDir); break;
            case 'status':       $base->orderBy('s.status', $orderDir); break;
            default:             $base->orderBy('users.id', $orderDir);
        }
